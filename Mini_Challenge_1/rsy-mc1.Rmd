...
100 - (nratings(MovieLense) / (dim(MovieLense)[1] * dim(MovieLense)[2]) * 100)

# Anzahl Ratings pro User
summary(rowCounts(MovieLense))
```
Jede Zeile von MovieLense entspricht einem Benutzer und jede Spalte einem Film. Gesamthaft gibt es 943 x 1664 = 1’569’152 Kombinationen zwischen User und Film. Allerdings hat nicht jeder Nutzer jeden Film gesehen, aus diesem Grund ist es wichtig die Sparisy Matrix zu betrachten. In MovieLense Matrix fehlen ca. 93% der Kombinationen. Ca. 7 % kann man einen Film einem User zuweisen. 
