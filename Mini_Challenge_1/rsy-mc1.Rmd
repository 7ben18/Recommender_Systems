...
set.seed(1234)
training <- MovieLenseCut %>% sample_frac(0.8)
training

test <- anti_join(MovieLenseCut, training, by = c("user", "item", "rating"))
test

training <- as(training, "realRatingMatrix")  
...
```{r}
ribcftopNList = predict(ribcf, test, n=10)
ribcftopNList
```
