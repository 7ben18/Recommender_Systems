...
ggplot(df_cosine_sim_matrix_long, aes(x = cosine_sim, color = )) + 
  geom_density() + 
  facet_wrap(~ user_id)
```
...
```{r}
get_top_N_list <- function(rating_matrix, N) {
...
get_top_N_list(movie_rating_matrix, 10)[c(5,718)]
```
...
```{r}
# testing
top_10_list <- get_top_N_list(movie_rating_matrix, 10)
...
for (user in 1:length(top_10_list)) {
  top_10_vec <- c(top_10_vec, as.numeric(top_10_list[[user]]))
}
...
plot_top_N_list <- function(rating_matrix, N) {
  top_N_list <- get_top_N_list(rating_matrix, N)
  top_N_vec <- c()

  for (user in 1:length(top_N_list)) {
    top_N_vec <- c(top_N_vec, as.numeric(top_N_list[[user]]))
  }
...
plot_top_N_list(movie_rating_matrix, 10)
...
plot_top_N_list(movie_rating_matrix, 10)
plot_top_N_list(movie_rating_matrix, 20)
plot_top_N_list(movie_rating_matrix, 50)
plot_top_N_list(movie_rating_matrix, 100)
...
```{r}
# testing
top_45_list <- get_top_N_list(movie_rating_matrix, 20)[c(5,25,50,150)]
...
```{r}
plot_top_N_user_barplot <- function(rating_matrix, N, user) {
  top_N_list <- get_top_N_list(rating_matrix, N)
  ...
  ggplot(df_N_user, aes(x = reorder(movies_N_user, sim_N_user), y = sim_N_user)) +
    geom_bar(stat = "identity", fill = "blue", alpha = 0.5) + 
...
plot_top_N_user_barplot(movie_rating_matrix, 20, 5)
plot_top_N_user_barplot(movie_rating_matrix, 20, 25)
plot_top_N_user_barplot(movie_rating_matrix, 20, 50)
plot_top_N_user_barplot(movie_rating_matrix, 20, 150)
```
...
get_top_N_list(rating_matrix, 20)[[133]]
```


```{r}
# testing
top_10_list_46 <- get_top_N_list(movie_rating_matrix, 10)

# create a empty vector 
top_10_vec_46 <- as.numeric(top_10_list_46[[133]])

# create now a density plot with ggplot 
ggplot(data.frame(top_10_vec_46), aes(x = top_10_vec_46)) + 
  geom_density(fill = "blue", alpha = 0.5) + 
  labs(title = "Density Plot top-10 List fuer User 133", 
       x = "Similarity", 
       y = "Density")


# create a function for density plot, as parameter 10 and 133
plot_top_N_list_46 <- function(rating_matrix, N, user) {
  top_N_list <- get_top_N_list(rating_matrix, N)
  top_N_vec <- as.numeric(top_N_list[[user]])

  ggplot(data.frame(top_N_vec), aes(x = top_N_vec)) + 
    geom_density(fill = "blue", alpha = 0.5) + 
    labs(title = paste("Density Plot top-", N, "List fuer User", user), 
         x = "Similarity", 
         y = "Density")
}

plot_top_N_list_46(movie_rating_matrix, 10, 133)
```

```{r}
plot_top_N_list_46(movie_rating_matrix, 20, 133)
plot_top_N_list_46(movie_rating_matrix, 30, 133)
plot_top_N_list_46(movie_rating_matrix, 40, 133)
plot_top_N_list_46(movie_rating_matrix, 50, 133)
```

```{r}
plot_top_N_list_46(movie_rating_matrix, 20, 555)
plot_top_N_list_46(movie_rating_matrix, 30, 555)
plot_top_N_list_46(movie_rating_matrix, 40, 555)
plot_top_N_list_46(movie_rating_matrix, 50, 555)
```
