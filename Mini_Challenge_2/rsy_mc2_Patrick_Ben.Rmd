...
```
Wir erkennen, dass bei der vollstaendiger Betrachtung der user_genre_matrix, dass wir 943 unterschiedliche Nutzerprofile aufweisen. Da die Dimension der user_genre_matrix 943 x 19 ist, ist somit jeder user unterschiedlich von einander. 
...
```
Bei der Cosine Similiarty erhalten wir Werte zwischen 0 und 1. Der Summary gibt uns einen nur Werte dazwischen aus. Ein Minimum von 0 bedeutet, dass es zwischen User und Film keine Aehnlichkeit vorhanden ist. Ein Hoher Wert deutet darauf hin, das User und Film eine hohe aehnlichkeit aufweisen. in der Cosine-Similarity gibt es Gesamthaft 1664 NAs, dies konnte ein User sein, der keinen Film Bewertet hat und somit man keine Cosine-Similarity fuer diesen User berechnen kann. Vermutung aufgrund der Dimension der Cosine-Sim-Matrix. Um Die Vermutung zu bestatigen musste man eine Zeile finde (user) der nur Na Werte aufweist. 
...
cosine_sim_matrix_df %>% 
  ...
  labs(title = "Dichteplot der Cosinus-Aehnlichkeit von Nutzern und Filmen", 
       x = "Cosinus-Aehnlichkeit", 
       y = "Density") 
...
Im genrellen haben alle 4 ausgewaehlten User sehr wenig Filme bewertet/gesehen. 
User 5 hat am meisten Filme bewertet. (1664 - 1489) Bewertungen hat der User 5 gemacht. Fuer diese User kann man somit weiter Filme recommenden.
...
```
Betrachten wir den ganzen Datensatz der User-Item Matrix "Negativabzuges", so erkennen wir, das es der minimalste Wert 929 ist, was bedeutet, das ein oder auch mehrere User, man noch maxmal 929 Filme empfehlen kann von den 1664 die in unserem gesamten Datensatz enthalten ist. Im Durchschnitt haben die User 1559 Filme nicht bewertet bzw nicht geschaut, welches wir durch unseren recommender entsprechend empfehlen koennten.
